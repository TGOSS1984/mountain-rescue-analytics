...
# returning the first global match. Format is (west_lon, north_lat,
# east_lon, south_lat) as Nominatim's viewbox expects.
REGION_VIEWBOXES = {
    "Peak District": (-2.05, 53.55, -1.45, 53.05),
    "Lake District": (-3.65, 54.75, -2.70, 54.30),
    "Snowdonia (Eryri)": (-4.30, 53.25, -3.60, 52.85),
}

REGION_QUERY_SUFFIX = {
    "Peak District": "Peak District, UK",
    "Lake District": "Lake District, Cumbria, UK",
    "Snowdonia (Eryri)": "Snowdonia, Gwynedd, UK",
}
...
def geocode_all(df, location_col="location_text", team_col="source_team_id"):
    geolocator = Nominatim(user_agent="mountain-rescue-analytics-portfolio-project")
    cache = _load_cache()

    lats, lons, statuses, confidences = [], [], [], []

    for location_text, team_id in zip(df[location_col], df[team_col]):
...
def main():
    ...
    print(f"geocoding {len(df)} incidents (cached results are skipped on re-runs)…")
    df = geocode_all(df)
